...
export class PokemonCard {
  static render({
    pokemon,
    onNavigate,
  }: PokemonCardProps & { onNavigate: (path: string) => void }) {
    const pokemonId = extractPokemonId(pokemon.url);
    const imageUrl = getPokemonImageUrl(pokemonId);

    const handleClick = () => {
      onNavigate(`/pokemon/${pokemon.name}`);
    };
...
export const PokemonCardComponent = ({ pokemon }: PokemonCardProps) => {
  const navigate = useNavigate();
  return PokemonCard.render({ pokemon, onNavigate: navigate });
};
